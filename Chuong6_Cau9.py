...
M = [int(x) for x in input("Nhập các số cách nhau bằng khoảng trắng: ").split()]

le = [x for x in M if x % 2 != 0]
chan = [x for x in M if x % 2 == 0]
nguyen_to = [x for x in M if KtrSoNguyenTo(x)]
khong_nguyen_to = [x for x in M if not KtrSoNguyenTo(x)]

print("Dòng 1 - Các số lẻ:", le, "→ Có", len(le), "số lẻ")
print("Dòng 2 - Các số chẵn:", chan, "→ Có", len(chan), "số chẵn")
print("Dòng 3 - Các số nguyên tố:", nguyen_to)
print("Dòng 4 - Các số không phải nguyên tố:", khong_nguyen_to)
